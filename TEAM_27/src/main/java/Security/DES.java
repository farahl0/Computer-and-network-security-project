...
public class DES {
    ...
    public String decrypt(String cipherTextHex, String keyHex) {
        //لكن نعكسو الترتيب encryption نفس دسكة ال 
        ...
        // Step 3: Run 16 rounds using subkeys
        for (int i = 15; i >= 0; i--) {
            String temp = R;
            R = xor(L, feistel(R, subkeys[i]));
            L = temp;
    ...
    private static String[] generateSubKeys(String keyBin) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            // PC-2: Combine and permute to 48-bit subkey
            subkeys[i] = permute(C + D, PC2);
        }
